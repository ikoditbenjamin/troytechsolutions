...
const bubbleVariants = {
  ...
  visible: { opacity: 1, scale: 1, y: 0, transition: { type: "spring" as const, stiffness: 380, damping: 22 } },
};
...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 1, role: "bot", text: "👋 Hi! I'm **TroyBot**, your TroyTech assistant. Ask me anything about our IT services, pricing, team, or location!", time: formatTime() },
  ]);
  ...
  const sendMessage = (text: string) => {
    ...
    if (!trimmed) return;
    const userMsg: Message = { id: msgCounter.current++, role: "user", text: trimmed, time: formatTime() };
    setMessages((prev) => [...prev, userMsg]);
    setInput("");
    setTyping(true);
    setTimeout(() => {
      const botMsg: Message = { id: msgCounter.current++, role: "bot", text: getAnswer(trimmed), time: formatTime() };
    ...
    <>
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-6 right-6 z-[9999] h-14 w-14 rounded-full flex items-center justify-center bg-gradient-to-br from-[#F97316] to-[#ea580c] shadow-[0_0_0_4px_rgba(249,115,22,0.2),0_8px_32px_rgba(249,115,22,0.45)] transition-shadow duration-300"
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            key="chatwindow"
            className={`fixed bottom-6 right-6 z-[9999] flex flex-col w-[360px] sm:w-[380px] rounded-2xl overflow-hidden bg-[#1a1a1a] border border-orange-500/20 shadow-[0_0_0_1px_rgba(249,115,22,0.1),0_24px_60px_rgba(0,0,0,0.6)] origin-bottom-right ${minimized ? "h-[64px]" : "h-[560px] max-h-[80vh]"}`}
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#F97316]/20 to-[#ea580c]/10 border-b border-orange-500/15 shrink-0">
              <div className="flex items-center gap-3">
                <motion.div
                  className="relative h-9 w-9 rounded-full bg-gradient-to-br from-[#F97316] to-[#ea580c] flex items-center justify-center shadow-[0_0_12px_rgba(249,115,22,0.5)]"
                  animate={{ boxShadow: ["0 0 12px rgba(249,115,22,0.5)", "0 0 22px rgba(249,115,22,0.8)", "0 0 12px rgba(249,115,22,0.5)"] }}
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-orange-500/20">
                    <AnimatePresence initial={false}>
                      {messages.map((msg) => (
                        ...
                        >
                          {/* Avatar */}
                          <div className={`shrink-0 h-7 w-7 rounded-full flex items-center justify-center mt-0.5 ${msg.role === "bot" ? "bg-gradient-to-br from-[#F97316] to-[#ea580c] shadow-[0_0_8px_rgba(249,115,22,0.4)]" : "bg-[#262626] border border-orange-500/20"}`}>
                            {msg.role === "bot" ? <Bot className="h-3.5 w-3.5 text-white" /> : <User className="h-3.5 w-3.5 text-orange-400" />}
                          </div>
                          {/* Bubble */}
                          <div className={`max-w-[78%] ${msg.role === "user" ? "items-end" : "items-start"} flex flex-col gap-1`}>
                            <div
                              className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${msg.role === "bot" ? "bg-[#262626] text-gray-200 rounded-tl-sm border border-orange-500/10" : "bg-gradient-to-br from-[#F97316] to-[#ea580c] text-white rounded-tr-sm shadow-[0_0_12px_rgba(249,115,22,0.3)]"}`}
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-orange-500/10 shrink-0">
                    <div className="flex items-center gap-2 bg-[#0a0a0a] border border-orange-500/20 rounded-xl px-3 py-2 focus-within:border-orange-500/50 focus-within:shadow-[0_0_12px_rgba(249,115,22,0.1)] transition-all duration-200">
                      ...
                      <motion.button
                        ...
                        className={`h-8 w-8 rounded-lg flex items-center justify-center transition-all duration-200 ${input.trim() && !typing ? "bg-[#F97316] text-white shadow-[0_0_10px_rgba(249,115,22,0.4)] hover:brightness-110" : "bg-gray-800 text-gray-600 cursor-not-allowed"}`}
                      >
                    ...
                    <p className="text-center text-[10px] text-gray-600 mt-2">
                      Powered by TroyTech ·{" "}
                      <a href="/contact" className="text-orange-500/70 hover:text-orange-400 transition-colors">Contact us</a>
                    </p>
                  </div>
                </motion.div>
              )}
